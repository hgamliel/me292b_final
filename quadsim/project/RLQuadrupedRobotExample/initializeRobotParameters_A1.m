...
L = 0.183*2;  % Distance b/w front and rear hip joints (m)
L_back = L;   % Length of the torso (m)
l1 = 0.2;     % Link #1 length (m)
l2 = 0.2;     % Link #2 length (m)

% Robot mass
M = 108/9.8;% Torso mass (kg)
m1 = 0.01;  % Leg link #1 mass (kg)
m2 = 0.01;  % Leg link #2 mass (kg)

% Moment of inertia (kg-m^2)
% Ixx = 1/12 * M * ((0.1*L_back)^2 + (0.1*L_back)^2);
% Iyy = 1/12 * M * (L_back^2 + (0.1*L_back)^2);
% Izz = 1/12 * M * (L_back^2 + (0.1*L_back)^2);
torso_MOI = [0.24, 0.8, 1];
...
% Ground properties
ground.stiffness = 1e12; % 1e3
ground.damping = 0; % 1e2
ground.length = 100;
ground.width = 1;
ground.depth = 0.05;
...
u_max = 55;                        % max joint torque = +/- u_max
y_min = 0.1;                       % min height of body from ground
...
q_hip_min = -240 * d2r;            % hip and knee joint angle limit
q_hip_max = 60 * d2r;
q_knee_min = 52.5 * d2r;
q_knee_max = 154.5 * d2r;
