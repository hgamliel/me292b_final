function [out,use_torque,kp,kd] = MPC(t,v,omega,quat,q,dq,foot_contact)
    ...
    % nlobj.Jacobian.StateFcn = @(x, u) QuadrupedStateFcn(x, u);
    % 
    % % specify sample time [s], prediction horizon [steps], control horizon [steps]
    % nlobj.Ts = 0.1;
    ...
    C = eye(13); D = zeros(13,12);
    QuadPlant = ss(Ac,Bc,C,D,-1);   % discrete-time dynamics
    mpcobj = mpc(QuadPlant);        % nonlinear MPC object
    ...
    % eliminate contact force from foot
    min_lim = [repmat({-inf},12,1)];
    max_lim = [repmat({inf},12,1)];
    ...
    mpcobj.Weights.OutputVariables = ...
        [[0.1, 0.1, 0.1] [0, 0, 0.5] [0.1, 0.1, 0.1] [1, 0.5, 0.5] 0];
    mpcobj.Weights.ManipulatedVariablesRate = 0.1*ones(1,12);
...
% set ground contact constraints for linear MPC object
function setGroundContactConstraints(mpcobj)
